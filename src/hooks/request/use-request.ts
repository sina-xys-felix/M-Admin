...
import type { IUseRequestOption, IUseRequestConfig } from './types';
// 引入返回值类型
import type { CustomResponse } from '@/utils/request/index';
...
const useRequest = <ParamType = any, PromiseRequestType = any, DataType = CustomResponse<PromiseRequestType>>(
  PromiseRequest: (p: ParamType) => Promise<DataType>,
  params: ParamType,
  opt?: IUseRequestOption<DataType>
): IUseRequestConfig<ParamType, DataType> => {
  ...
  if (option.throttle && option.debounce) {
    console.warn('useRequest的配置项中的throttle和debounce均为true，请选择一个，否则这样默认使用防抖');
  }
