      ...
      <div class="example-container-right-side">
        ...
        <div class="example-container-right-side__content">
          ...
          <template v-if="current === 6">
            <a-space direction="vertical" fill :size="[16, 16]">
              <a-space> 性别：
                <MDict v-model="dictValue" :code="DictEnum.GENDER" style="width: 160px" />
              </a-space>
              <a-space> 民族：
                <MDict v-model="dictValue1" :code="DictEnum.NATION" style="width: 160px" />
              </a-space>
              <a-space>
                婚姻状况：
                <MDict v-model="dictValue2" :code="DictEnum.PARTY" allow-clear style="width: 160px" />
              </a-space>
              <a-space>
                婚姻状况（禁用）：
                <MDict v-model="dictValue2" disabled :code="DictEnum.PARTY" allow-clear />
              </a-space>
            </a-space>
          </template>
          ...
          <template v-if="current === 8">
            <a-space direction="vertical" fill :size="[16, 16]">
              <MForm ref="mFormRef" :form-data="mFormData" :columns="mFormSearchColumns"
                :labelColProps="{ flex: '120px' }" :wrapperColProps="{ flex: 1 }" />
          ...
          <template v-if="current === 10">
            <a-space direction="vertical" fill :size="[16, 16]">
              ...
              <MSearchForm ref="MSearchFormRef" :search-columns="searchColumns" :search-param="formData"
                :show-expand="false" :search="onSearch" :reset="onReset" />
          ...
          <template v-if="current === 16">
            <a-space fill direction="vertical">
              <a-alert> 采用开源视频播放器：<a-link href="https://h5player.bytedance.com/"
                  target="_blank">西瓜播放器</a-link></a-alert>
              <div class="player-box">
                <div id="video"></div>
              </div>
            </a-space>
          </template>
          <template v-if="current === 17">
            <a-space fill direction="vertical" :size="[16, 16]">
              <div>
                <a-space>
                  自定义敏感词：
                  <a-tag v-for="item in mintWords" color="orangered">{{ item }}</a-tag>
                </a-space>
              </div>
              <div>
                请输入一段话：<a-input v-model="mintInput" placeholder="输入一段话" @input="onInput" style="width:320px" />
              </div>
              <div>
                输出结果： {{ mintInput }}
              </div>
          ...
          <template v-if="current === 18">
            <a-space fill direction="vertical" :size="[16, 16]">
              <MTypeWriter :options="typeItOptions">
...
import MTypeWriter from '@/components/m-type-writer/index.vue'

const current = ref(1)

const typeItOptions = ref({
  strings: ['Hello World!', 'This is TypeIt!', 'M stands for Modular, symbolizing the modular design of the framework.'],
  speed: 100,
...
const list = computed(() => [
  ...
  {
    key: '17',
    title: 'components.mint.filter',
  },
  {
    key: '18',
    title: 'components.typeit',
  },
])
...
const mintWords = ["脑残", "废物", "白痴", "傻逼", "智障"];

const mintInstance = new Mint(mintWords)
...
const onInput = () => {
  mintInput.value = mintInstance.filter(mintInput.value).text;
}
