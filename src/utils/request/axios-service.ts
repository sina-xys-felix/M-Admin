...
import { AxiosRequestConfigRetry, RequestOptions, Result } from './type';
import { ContentTypeEnum } from '@/common/enums/http-enum';
...
export class VAxios {
  ...
  download<T = any>(config: AxiosRequestConfig, options?: RequestOptions): Promise<BlobPart> {
    return this.request({ ...config, method: 'POST' }, { ...options, responseType: 'blob' });
  }
